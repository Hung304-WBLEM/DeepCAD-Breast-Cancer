...
class Mass_Shape_Dataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        BREAST_MASS_SHAPES = ['ROUND', 'OVAL', 'IRREGULAR',
        ...
        for idx, mass_shape in enumerate(BREAST_MASS_SHAPES):
...
class Mass_Margins_Dataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        BREAST_MASS_MARGINS = ['ILL_DEFINED',
                               'CIRCUMSCRIBED',
                               'SPICULATED',
                               'MICROLOBULATED',
                               'OBSCURED']
        ...
        for idx, mass_margins in enumerate(BREAST_MASS_MARGINS):
...
class Calc_Type_Dataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        BREAST_CALC_TYPES = ["AMORPHOUS", "PUNCTATE","VASCULAR",
        ...
        for idx, calc_type in enumerate(BREAST_CALC_TYPES):
...
class Calc_Dist_Dataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        BREAST_CALC_DISTS = np.array(["CLUSTERED",
                                      "LINEAR",
                                      "REGIONAL",
                                      "DIFFUSELY_SCATTERED",
                                      "SEGMENTAL"])
        ...
        for idx, calc_dist in enumerate(BREAST_CALC_DISTS):
...
class Breast_Density_Dataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        for idx, breast_density in enumerate(BREAST_DENSITY_TYPES):
            images = glob.glob(os.path.join(root_dir, str(breast_density), '*.png'))
            self.images_list += images
            self.labels += [idx] * len(images)
...
class Pathology_Dataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        for idx, pathology in enumerate(PATHOLOGY):
...
class Features_Pathology_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.lesion_type == 'mass':
            ...
            feature_vector = Features_Pathology_Dataset.convert_mass_feats_1hot(breast_density, mass_shape, mass_margins)
            ...
            feature_vector = Features_Pathology_Dataset.convert_calc_feats_1hot(breast_density, calc_type, calc_distribution)
