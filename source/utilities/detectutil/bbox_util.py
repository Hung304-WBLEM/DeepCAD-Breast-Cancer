...
def extendBB(org_img_size, left, top, right, bottom, ratio=0.3):
    ...
    return max(0, int(center_x - new_width/2)), max(0, int(center_y - new_height/2)), \
        min(org_img_size[1], int(center_x + new_width/2)
            ), min(org_img_size[0], int(center_y + new_height/2))
